...
if ( !class_exists( 'PBB_Admin' ) &&  is_admin() ){
	...
	class PBB_Admin  {
		...
		function build_info_screen(){
			...
			echo 'Attach individual bars to categories or even to individual posts. Customize your message to the exact visitors you are trying to reach.';
			echo '</p><p>';
			...
			echo '<p>'.__("PBB tracks events to your Google Analytics account directly. We piggyback on the code you probably already use. Read BLOGPOST for more about the power of Google Tracking Events. Google Analytics always tracks the page URL along with the event. Each of these are visible in the Google Analytics account screens.",'pbb-textdomain').'</p>';
			...
			echo '</ul>';
			echo '<p>'.__("Check out this BLOGPOST for more about using the Events. Currently the PeanutButterBar tracks only with Google Analytics. Check out this POST for more on additional tracking support.",'pbb-textdomain').'</p>';
			echo '</div></div></div>';
			echo '<div class="postbox-container" style="margin: 10px 0 0 0;">';
			echo '<div class="postbox">';
			echo '<h3 style="padding:12px; cursor:auto;">'.__("Contact Us",'pbb-textdomain').'</h3>';
			echo '<div class="inside">';
			echo '<ul>';
			echo '<li>Visit the <a target="_blank" href="http://peanutbutterplugin.com/?utm_source=adminlink&utm_medium=bar">plugin page</a></li>';
